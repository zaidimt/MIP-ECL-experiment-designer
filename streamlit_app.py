...
exact_match = df[
    (np.isclose(df['moles_surf.proteins'], moles, rtol=0.05)) &
    (np.isclose(df['capture'], capture, rtol=0.05)) &
    (np.isclose(df['probe'], probe, rtol=0.05)) &
    (np.isclose(df['Affinity_kD'], kd, rtol=0.05))
]
